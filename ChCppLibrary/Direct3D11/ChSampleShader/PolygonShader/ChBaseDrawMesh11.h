...
namespace ChD3D11
{
	namespace Shader
	{
		...
		private://Member Value//
			...
			BaseDrawMesh11CRT* value = nullptr;
		};
	}
}
...
void ChD3D11::Shader::BaseDrawMesh11<CharaType>::DrawMain(ChCpp::FrameObject<CharaType>& _object)
{
	...
	value->drawDatas[frame].push_back(drawData);
}
...
void ChD3D11::Shader::BaseDrawMesh11<CharaType>::DrawEnd()
{
	...
	for (auto&& drawData : value->drawDatas)
	{
		...
		for (auto&& prim : primitiveData)
		{
			...
			auto&& mate11 = *prim->ValueIns().mate;
			...
			for (auto&& drawFrame : drawData.second)
			{
				...
				polyData.SetBaseTexture(prim->ValueIns().textures[Ch3D::TextureType::Diffuse].get());
				polyData.SetNormalTexture(prim->ValueIns().textures[Ch3D::TextureType::Normal].get());

				polyData.SetShaderTexture(GetDC());

				GetDC()->DrawIndexed(static_cast<unsigned int>(prim->ValueIns().indexArray.size()), 0, 0);
			}
		}
	}
	if (!value->drawDatas.empty())value->drawDatas.clear();
